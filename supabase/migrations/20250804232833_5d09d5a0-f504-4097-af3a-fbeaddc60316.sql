...
BEGIN
  ...
  ELSE
    ...
    DECLARE
      weekly_amount INTEGER := provider_amount / CEIL(rental_days::float / 7);
      current_date TIMESTAMPTZ := p_start_date;
      remaining_amount INTEGER := provider_amount;
    BEGIN
        ...
        IF remaining_amount <= weekly_amount THEN
          ...
          release_schedule := release_schedule || jsonb_build_array(
            jsonb_build_object(
              ...
              'release_date', LEAST(current_date + INTERVAL '7 days', p_end_date),
              'type', 'weekly'
            )
          );
          EXIT;
